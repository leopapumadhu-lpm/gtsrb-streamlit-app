...
.prediction-card {background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); padding:1rem; border-radius:15px; color:white; margin:1rem 0;}
...
def get_meaning(class_id, lang='en'):
    if class_id in class_names:
        return class_names[class_id].get(lang, class_names[class_id]['en'])
    return "Unknown"

def speak_text_cloud(text, lang='en'):
...
# -------------------- Download --------------------
def get_download_link(img, name="result.png"):
    buffered = BytesIO()
    img.save(buffered, format="PNG")
    img_str = base64.b64encode(buffered.getvalue()).decode()
    return f'<a href="data:file/png;base64,{img_str}" download="{name}" style="background-color:#4CAF50;color:white;padding:10px 15px;text-decoration:none;border-radius:5px;">📥 Download</a>'
...
if tab=="Home":
    ...
    if uploaded_file and model:
        ...
        if st.button("Analyze"):
            ...
            for i, idx in enumerate(top_indices):
                ...
                st.markdown(f'<div class="bar" style="width:{conf*100}%; background-color:#2196F3;"></div>', unsafe_allow_html=True)
            
            # Speak top prediction
            speak_text_cloud(get_meaning(top_indices[0], lang), lang=lang)
            
            # Download cropped image
            st.markdown(get_download_link(img_crop), unsafe_allow_html=True)

elif tab=="Statistics":
    st.markdown('<h1 class="main-header">📊 Statistics</h1>', unsafe_allow_html=True)
    st.markdown("### Sample Class Distribution")
    sample = list(class_names.keys())[:10]
    counts = np.random.randint(50,500,len(sample))
    fig,ax = plt.subplots(figsize=(8,4))
    ax.bar([class_names[i]['en'] for i in sample], counts, color='skyblue')
    ...
    st.markdown("""
    This app recognizes 43 German traffic signs using CNN with:
    - Safe auto-cropping
    - Multi-language meaning (EN, TA, HI)
    - Top-K predictions
    - Voice explanation
    - Statistics & Modern UI
    """)
